...
![a compass with two axis: no agency (left) to full agency (right) on the horizontal axis, and autonomous (bottom) to conversational (top) on the vertical axis.](empty-compass-inv.png)
...
![Diagram of the operation of a direct LLM call: a user asks a question to an LLM and the LLM replies directly.](direct-llm-call-inv.png)
...
![the updated compass](direct-llm-call-compass-inv.png)
...
![Diagram of the operation of a RAG app: first the user question is sent to a retriever system, which fetches some additional data relevant to the question. Then, the question and the additional data is sent to the LLM to formulate an answer.](basic-rag-inv.png)
...
![the updated compass](basic-rag-compass-inv.png)
...
![Diagram of the operation of an agentic RAG app: when the user asks a question, before calling the retriever the app checks whether the retrieval step is necessary at all. Once the check is done, if retrieval was necessary it is run, otherwise the app skips directly to the LLM, which then replies to the user.](agentic-rag-inv.png)
...
![the updated compass](agentic-rag-compass-inv.png)
...
![the updated compass](llm-with-function-calling-compass-inv.png)
...
![the updated compass](self-correcting-rag-compass-inv.png)
...
![Diagram of the operation of a chain-of-thought LLM app: when a user asks a question, first of all the LLM reacts to the chain-of-thought prompt to lay out the sub-questions it needs to answer. Then it answers its own questions one by one, asking itself each time whether the final answer has already been found. When the LLM believes it has the final answer, it rewrites it for the user and returns it ](chain-of-thought-inv.png)
...
![the updated compass](chain-of-thought-compass-inv.png)
...
![the updated compass](multi-hop-rag-compass-inv.png)
...
![the updated compass](react-agent-compass-inv.png)
...
![the updated compass](conversational-agent-compass-inv.png)
...
![the updated compass](ai-crews-compass-inv.png)
