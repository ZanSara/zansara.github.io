...
date: 2024-06-10
author: "ZanSara"
featured-image: "/posts/2024-06-10-the-agent-compass/cover-inv.png"
---
...
![a compass with two axis: no agency (left) to full agency (right) on the horizontal axis, and autonomous (bottom) to conversational (top) on the vertical axis.](/posts/2024-06-10-the-agent-compass/empty-compass-inv.png)
...
![Diagram of the operation of a direct LLM call: a user asks a question to an LLM and the LLM replies directly.](/posts/2024-06-10-the-agent-compass/direct-llm-call-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/direct-llm-call-compass-inv.png)
...
![Diagram of the operation of a RAG app: first the user question is sent to a retriever system, which fetches some additional data relevant to the question. Then, the question and the additional data is sent to the LLM to formulate an answer.](/posts/2024-06-10-the-agent-compass/basic-rag-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/basic-rag-compass-inv.png)
...
![Diagram of the operation of an agentic RAG app: when the user asks a question, before calling the retriever the app checks whether the retrieval step is necessary at all. Once the check is done, if retrieval was necessary it is run, otherwise the app skips directly to the LLM, which then replies to the user.](/posts/2024-06-10-the-agent-compass/agentic-rag-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/agentic-rag-compass-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/llm-with-function-calling-compass-inv.png)
...
![Diagram of the operation of self correcting RAG: when the user asks a question, the retriever is called and the results are sent to the LLM to extract an answer from. However, before returning the answer to the user, another LLM is asked to judge whether in their opinion, the answer is correct. If the second LLM agrees, the answer is sent to the user. If not, the second LLM generates a new question for the retriever and runs it again, or in other cases, it simply integrates its opinion in the prompt and runs the first LLM again.](/posts/2024-06-10-the-agent-compass/self-correcting-rag-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/self-correcting-rag-compass-inv.png)
...
![Diagram of the operation of a chain-of-thought LLM app: when a user asks a question, first of all the LLM reacts to the chain-of-thought prompt to lay out the sub-questions it needs to answer. Then it answers its own questions one by one, asking itself each time whether the final answer has already been found. When the LLM believes it has the final answer, it rewrites it for the user and returns it ](/posts/2024-06-10-the-agent-compass/chain-of-thought-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/chain-of-thought-compass-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/multi-hop-rag-compass-inv.png)
...
![Diagram of the operation of a ReAct Agent: when user sets a goal, the app builds a ReAct prompt, which first of all asks the LLM whether the answer is already known. If the LLM says no, the prompt makes it select a tool. The tool returns some values which are added to the inner monologue of the application toghether with the invitation to re-assess whether the goal has been accomplished. The app loops over until the answer is found, and then the answer is returned to the user.](/posts/2024-06-10-the-agent-compass/react-agent-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/react-agent-compass-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/conversational-agent-compass-inv.png)
...
![the updated compass](/posts/2024-06-10-the-agent-compass/ai-crews-compass-inv.png)
