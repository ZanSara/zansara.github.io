...
That's broadly correct. Temperature is a parameter of the LLM final decoding steps, the only one in the whole Transformer architecture that truly incorporates some randomness by design. At this stage, once the model calculated the logits of the next token candidates, the model has to map that value to an actual token from a list. Normally, LLMs perform best when they're allowed to pick not necessarily the best token, but when they can choose among the N best tokens at random: the size of N is, more or less, what the temperature parameter represents.

Therefore, when we set the temperature to 0, the LLM must always choose the best next token, without taking random choices. So, if the input is fixed and we removed the only source of randomness in the architecture, the outputs should be always identical... right?

And yet, in practice, it often is not. Run the same prompt twice, with the same model, same parameters, and temperature 0, and sooner or later the output will be a bit different. Not by much, usually. It will start with just a word, then the sentence takes a slightly different spin, until eventually the rest of the completion drifts away.
...
You can think of it as: **sampling determinism** is not the same thing as **system determinism**.

## It gets worse

However, this is only part of the problem. You may already be objecting that running the same matrix multiplication on a GPU on the same data repeatedly will always provide bitwise equal results. The computations are done in floating-point arithmetic, and there are surely other jobs running on the GPU while your computer is on. So why these calculations are deterministic, while LLM sampling with `temperature=0` is not?
...
To reduce randomness and make LLM outputs reproducible, some people recommend using a fixed seed, and indeed some providers expose one. OpenAI, for example, [documents](https://developers.openai.com/cookbook/examples/reproducible_outputs_with_the_seed_parameter) a `seed` parameter and says it makes a best effort to sample deterministically, while explicitly warning that determinism is not guaranteed and that backend changes can still affect outputs. Their `system_fingerprint` field exists precisely so you can notice when the underlying serving configuration changed.

The problem with fixed seeds is that they help reproduce results when the temperature is above zero, not when it's already zeroed out. That's because the fixed seed is fixing the randomness of the sampling step: by setting the temperature to zero we're already removing that source of randomness, so the net result is identical with ot without fixed seed, and every other source of randomness that comes from the GPU and the rest of the stack is unaffected. 

So fixed seeds are worth using when you are trying to get the same results for a call with non-zero temperature, like for tests, demos, and regression checks. But you must keep in mind that they only affect the sampler, and they won't help you when temperature is zero.
...
Caching doesn't exactly address the reproducibility issue per se, but in many applications it's the right level of abstraction where you want the same input to produce the same output. It's often not only the most viable option, but also the cheapest, simplest, and fastest, unless you're running a benchmark or an evaluation.
...
I get it, nobody likes such a huge, random black box in the core of our application's business logic. But removing randomness from the outputs is not the right way to manage an LLM's output. If you need completely deterministic output, rather use the LLM to design a decision tree (or a more sophisticted model, if you need it) and use that in your application.
